...
def gefs_stats():
    ...
    bmus_mems = np.empty((int(len(obsv.index)),len(glob.glob('data/z-gefs/gefs-z-*'))))
    ...
    for gefs_file in glob.glob('data/z-gefs/gefs-z-*'):
        ...
        date = slice(obsv.index.values[0],obsv.index.values[-1])
    ...
    dist_xr = xr.DataArray(data=bmus_mems,dims=['index','member'],
                           coords=dict(index=obsv.index,member=members))
    ...
    if res < res_obs:  # do some resampling, avging of shorter res to get longer res forecasts
        ...
        obsv_loc = obsv.resample(index=res_obs_str).mean()
...
if __name__ ==  "__main__":
    ...
    title = '6h-anomalies-som-500-members-crpss'
    ...
    for dom in range(len(lat_dif)):  # for each domain size
        ...
        era = xr.open_dataset('era-2009-2022.nc',engine='scipy').sel(level=500,latitude=lat,longitude=lon-360)
